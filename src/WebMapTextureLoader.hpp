...
class WebMapTextureLoader {
 public:
  ...
  std::future<std::string> loadTextureAsync(std::string time, std::string requestStr,
      std::string const& centerName, std::string const& format, std::string const& mapCache);

  /// WMS texture loader.
  std::string loadTexture(std::string time, std::string requestStr, std::string const& name,
      std::string const& format, std::string const& mapCache);
